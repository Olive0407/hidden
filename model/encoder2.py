...
class Encoder(nn.Module):
    ...
    def __init__(self, config: HiDDenConfiguration):
        ...
        self.conv_layers = nn.Sequential(*layers)     #列表之前带星号，将列表拆成独立的元素
        self.after_concat_layer = ConvBNRelu(self.conv_channels + 3 + config.message_length,
                                             self.conv_channels)     #30+3+64,64
    ...
    def forward(self, image, message):
        ...
        expanded_message = message.unsqueeze(-1)    #8 30 1
        expanded_message.unsqueeze_(-1)  # 8 30 1 1

        expanded_message = expanded_message.expand(-1,-1, self.H, self.W)  #8 30 128 128
